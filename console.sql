...
show create table default.student;
--
...
--
-- select named_struct("id",1001,"name","zhangsan")
...
-- 2）数据装载
-- （1）首日装载
-- date_format(data.using_time,'yyyy-MM-dd') date_id,下单过程的时间来自下单时间
-- data.using_time is not null;使用的下单操作
...
-- （2）每日装载
-- and array_contains(map_keys(old),'using_time');保证本次修改的字段有using_time
...
-- 2）数据装载
-- （1）首日装载
-- and data.used_time is not null;已经使用(支付)
...
-- 2）数据装载
-- from ods_log_inc lateral view explode(actions) tmp as action 炸裂函数，
set hive.cbo.enable=false;
...
(
    ...
    where dt>=date_add('2020-06-14',-7) -- 或者使用date_sub('2020-06-14',7)[即把-号去掉]
    and dt<'2020-06-14'
)t1
join
(
